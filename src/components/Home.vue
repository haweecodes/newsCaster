<template>
  <v-app>
    <v-navigation-drawer clipped fixed v-model="drawer" app  class="blue lighten-3" dark >
               <v-list>
      ...
      <v-toolbar app fixed clipped-left class="blue lighten-3" dark>
        ...
        <v-toolbar-items class="hidden-sm-and-down">
           ...
           <v-layout>
             <v-flex xs12>
            <v-text-field label="Search News" append-icon="search" v-model="search" placeholder="Enter To Search" @keyup.enter="searchEntered(search)"></v-text-field>
             </v-flex>
           </v-layout>
            <!-- <v-menu>
          ...
          <v-flex xs12 lg4 v-for="(item, i) in articles" :key=i>
             <v-card>
              ...
              <v-card-actions text-align="right">
                <v-spacer></v-spacer>
                <v-btn flat color="orange" @click="window.location = item.url">View</v-btn>
    ...
    this.$store.dispatch('getNews').then(function (response) {
      ...
      vue.articles.forEach(element => {
        if(element.urlToImage === null) {
          element.urlToImage = 'https://vignette.wikia.nocookie.net/simpsons/images/6/60/No_Image_Available.png/revision/20170219125728'
        }
      })
    })
  },
